...
import fs from "fs";
import { selectModel, type Model } from "../models/select.js";
...
export async function createGitCommit(diff: string) {
  if (fs.existsSync(preferencesFilePath)) {
    ...
    try {
      ...
      generalError(
        `An error occured while creating Git commit with ${preferredModel} :  + ${error}`
      );
    }
  } else {
    selectModel();
