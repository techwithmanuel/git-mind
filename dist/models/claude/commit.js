...
export async function generateClaudeCommit(input) {
        ...
        try {
            ...
            const result = await genAI.messages.create({
                ...
                messages: [
                    {
                        role: "user",
                        content: base + `the git diff is ${input}`,
                    },
                ],
            ...
            exec("git reset");
            cancel(chalk.red("An error occurred while generating the Claude response:", error));
        }
    }
    else {
